...
impl Genome for FeedForwardGenome {
    ...
    fn add_conn_mut(&mut self, config: &Self::Config) -> Self::ConnGene {
        ...
        assert_ne!(possible_in_nodes.len(), 0, "cannot add connection if there are no present in nodes");
        ...
        assert_ne!(possible_out_nodes.len(), 0, "cannot add connection if there are no present out nodes");
        ...
        ).expect("the existence of at least one possible in node has already been verified");
        
        let out_node = possible_out_nodes.iter().find(|node|
            node.backward(|backward| !backward.iter().any(|conn| conn.in_node().eq(in_node)))
        ).expect("the existence of at least one possible out node has already been verified");

        let new_conn = ConnGene::new(in_node.clone(), out_node.clone(), rand::random(), config.innov().next());
        new_conn.in_node().forward_mut(|forward| forward.insert(new_conn.clone()));
    ...
    fn add_node_mut(&mut self, config: &Self::Config) -> Self::NodeGene {
        assert_ne!(self.conn_genes.len(), 0, "cannot add node if there are no connections to split");
        ...
        let conn_a = ConnGene::new(old_conn.in_node(), new_node.clone(), 1.0, config.innov().next());
        let conn_b = ConnGene::new(new_node.clone(), old_conn.out_node(), old_conn.weight(), config.innov().next());
    ...
    fn activate(&self, input: Vec<f32>, config: &Self::Config) -> Vec<f32> {
        ...
        let mut map = BTreeMap::from_iter(self.input_nodes.iter().enumerate().map(|(i, node)|
            (node.clone(), input.get(i).cloned().expect("the number of input nodes is constant"))
        ));
